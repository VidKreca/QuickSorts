...
partition = (arr, bottom, top) => {
	let m = arr[Math.floor((bottom+top) / 2)]
	let l = bottom-1, r = top+1
...
qs = (arr, bottom, top) => {
	if (bottom < top) {
		let j = partition(arr, bottom, top)
		qs(arr, bottom, j)
		qs(arr, j+1, top)
	}
}
