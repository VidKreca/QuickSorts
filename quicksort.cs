...
class Program
{
    static uint partition<T>(List<T> arr, uint bottom, uint top) where T : System.IComparable<T>
    {
        T m = arr[Convert.ToInt32((bottom + top) / 2)];
        int l = (int)bottom - 1, r = (int)top + 1;
        ...
        while (true)
        {
            ...
            {
                l++;
            } while (arr[l].CompareTo(m) > 0 && l <= top);
            do
            {
                r--;
            } while (arr[r].CompareTo(m) < 0 && r >= bottom);
    ...
    static void qs<T>(List<T> arr, uint bottom, uint top) where T : System.IComparable<T>
    {
        uint j;
    ...
    static void QuickSort<T>(List<T> arr) where T : System.IComparable<T>
    {
        qs(arr, 0, Convert.ToUInt32(arr.Count)-1);
    }
    ...
    static void Main(string[] args)
    {
        ...
        double sum = 0;
        foreach (double x in timings)
            sum += x;
        double avg = sum / n;

        Console.WriteLine("Average time:   " + avg.ToString() + "s");
        Console.ReadLine();
    }
}
