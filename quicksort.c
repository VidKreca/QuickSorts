...
/*
    QuickSort for int arrays only

    Compiled with: gcc -Ofast
*/

unsigned int partition(int* arr, unsigned int bottom, unsigned int top) {
    int m = arr[(bottom + top) / 2];
...
void qs(int* arr, unsigned int bottom, unsigned int top) {
    unsigned int j;
...
void QuickSort(int* arr, unsigned int length) {
    qs(arr, 0, length-1);
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        double duration = ((et.tv_sec - st.tv_sec)*1000000000) + (et.tv_nsec - st.tv_nsec);   // ns
        timings[c++] = duration;
        printf("%f,  ", duration);
    }
